    ...
    class SystrayClientPlugin(BasePlugin):
        ...
        async def fetch_menu_path(self, service_name):
            ...
            try:
                ...
                properties = proxy_object.get_interface(
        ...
        async def initialize_proxy_object(self, service_name):
            ...
            try:
                ...
                try:
                    self.introspection = await bus.introspect(
                        service_name, self.menu_path
                    )
                except Exception as e:
                    self.logger.error(
        ...
        def create_menu_item(
            self, menu, name, label_index, service_name, panel_instance
        ):
            ...
            action.connect(
                "activate",
                self._on_menu_item_clicked_wrapper,
                label_index,
                service_name,
            )
