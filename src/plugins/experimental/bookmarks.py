...
class PopoverBookmarks(BasePlugin):
    ...
    def on_start(self):
        ...
        self.run_in_async_task(self._load_and_update_bookmarks(is_initial_load=True))
    ...
    def open_popover_bookmarks(self, *_):
        if self.popover_bookmarks and self.popover_bookmarks.get_property("visible"):
    ...
    def _create_round_thumbnail(self, source_path: str, target_path: str) -> bool:
        ...
        try:
            ...
            with Image.open(source_path).convert("RGBA") as icon_img:
                ...
                final_img = Image.new("RGBA", self.THUMBNAIL_SIZE, (255, 255, 255, 0))
    ...
    def _load_raw_bookmarks(self) -> list[Dict[str, Any]]:
        path_handler = PathHandler(self._panel_instance)
        bookmarks_path = path_handler.get_data_path()
        ...
        for name, bookmark_data in all_bookmarks.items():
            ...
            bookmark_image = self.os.path.join(self.bookmarks_image_path, safe_filename)
        ...
        if should_download:
            ...
            if isinstance(download_result, tuple) and len(download_result) == 2:
                ...
                try:
                    ...
                    self.logger.warning(f"Failed to create marker file for {url}: {e}")
            else:
                download_type = "scraped"
                ...
                if is_round_icon:
                    ...
                    try:
                        with Image.open(bookmark_image) as img:
                            ...
                            img.save(
                                thumbnail_path, "PNG", quality=self.THUMBNAIL_QUALITY
                            )
    ...
    def _update_popover_ui(self, processed_bookmarks, is_initial_load: bool = False):
        if not self.popover_bookmarks:
            return False
