...
def get_plugin_class():
    ...
    class MullvadPlugin(BasePlugin):
        ...
        async def _async_init_setup(self):
            ...
            """
            self.mullvad_version = await self.asyncio.to_thread(
                self.get_mullvad_version
            )
            self.icon_name = self.gtk_helper.icon_exist(
                "mullvad-vpn",
                [
                    "mullvad-vpn-symbolic",
                    "mullvaddg",
        ...
        async def set_mullvad_relay_by_city(self, *_):
            ...
            try:
                ...
                city_relays = [
                    r for r in relays if r.get("city_code") == self.city_code
                ]
                if not city_relays:
                    raise RuntimeError(
                        f"No relays found for city code '{self.city_code}'."
                    )
        ...
        async def update_vpn_status(self):
            ...
            try:
                net_files = await self.asyncio.to_thread(
                    self.os.listdir, "/sys/class/net"
                )
                is_mullvad_active = any(
                    (file.startswith("wg") or file.startswith("tun"))
                    for file in net_files
                )
                if not is_mullvad_active:
                    is_mullvad_active = any(
                        file.startswith("tun") and "-mullvad" in file
                        for file in net_files
                    )
