...
class RecordingPlugin(BasePlugin):
    ...
    def open_popover(self, widget):
        if self.popover and self.popover.is_visible():
            ...
            self.create_popover()
            self.popover.popup()  # pyright: ignore
    ...
    def create_popover(self):
        ...
        self.popover.set_parent(self.button)  # pyright: ignore
    ...
    async def on_record_output_clicked(self, output_name):
        ...
        try:
            ...
            self.logger.exception(f"Failed to start wf-recorder for {output_name}: {e}")
    ...
    async def _join_with_ffmpeg(self):
        ...
        if not isinstance(outputs, list) or not all(
            ...
            return
        if not all(
            "geometry" in output and "height" in output["geometry"]
            for output in outputs
        ):
            self.logger.error(
    ...
    def code_explanation(self):
        ...
        utilities.
        **Refactoring Notes:**
        1. **Concurrency:** The explicit import of `global_loop` was removed, and
           all task scheduling now correctly uses the inherited instance attribute
           `self.global_loop.create_task()`, as the base class handles its
           initialization.
        2. **Exception Handling:** Most generic `except Exception` blocks now use
           `self.logger.exception()` instead of `self.logger.error()`. This ensures
           that the full Python traceback is automatically included in the log,
           making unexpected errors far easier to debug. For expected environmental
           issues (`FileNotFoundError` for `slurp` or `ffmpeg`) and timeouts,
           `self.logger.error()` is still used, as a full traceback is less useful.
