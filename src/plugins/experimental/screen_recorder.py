...
class RecordingPopover(Gtk.Box):
    ...
    def __init__(self, main_plugin):
        super().__init__(orientation=main_plugin.gtk.Orientation.VERTICAL, spacing=6)
    ...
    def build_ui(self):
        ...
        self.audio_switch.connect("state-set", self.main_plugin.on_audio_switch_toggled)
...
class RecordingPlugin(BasePlugin):
    ...
    async def on_stop_and_join_clicked(self):
        ...
        valid_output_files = [f for f in self.output_files if self.os.path.exists(f)]
    ...
    async def _join_with_ffmpeg(self):
        ...
        for i in range(num_outputs):
            filter_complex_parts.append(f"[{i}:v]scale=-1:{min_height},setsar=1[v{i}]")
            input_video_streams += f"[v{i}]"
        if num_outputs > 1:
            hstack_chain = f"hstack=inputs={num_outputs}"
            filter_complex_parts.append(f"{input_video_streams}{hstack_chain}[v_out]")
        elif num_outputs == 1:
        ...
        try:
            ...
            self.notifier.notify_send("Join Failed", "ffmpeg not installed.", "record")
        except Exception as e:
