from ._package_manager import PackageHelper
...
class AppMenuHandler:
    ...
    def on_right_click_popover(self, gesture, n_press, x, y, vbox):
        ...
        if self.pkg_helper.is_supported():
            ...
            uninst_btn.connect(
                "clicked", self.on_uninstall_clicked, desktop_file, popover
            )
    ...
    def remove_from_dockbar(self, button, desktop_file, popover):
        ...
        apps = config.get("app", {})
        key = next(
            (k for k, v in apps.items() if v.get("desktop_file") == desktop_file), None
        )
        if key:
            del apps[key]
            config["app"] = apps
            self.plugin.config_handler.set_root_setting(
                [self.plugin.dockbar_id], config
            )
        popover.popdown()
    ...
    def open_desktop_file(self, button, desktop_file, popover):
        ...
        target = next(
            (
                os.path.join(l, desktop_file)
                for l in locations
                if os.path.exists(os.path.join(l, desktop_file))
            ),
            None,
        )
